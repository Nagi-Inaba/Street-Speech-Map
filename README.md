...
- ✅ 候補者CRUD（管理画面：一覧・新規作成・編集）と候補者一覧（公開画面）
- ✅ 演説イベントCRUD（管理画面：一覧・新規作成・編集）と候補者ページの予定一覧（公開画面）
- ✅ Leaflet地図コンポーネント（イベントピン表示、入力UI、編集可能）
- ✅ 公開リクエスト投稿API（`/api/public/requests`）
- ✅ 報告API（開始/終了/場所変更）（`/api/public/reports`）
- ✅ リクエスト審査画面（重複キーによるグループ化表示、一括承認・却下機能）
- ✅ 共有ボタン（予定/実施中、Web Share API + クリップボードコピー）
- ✅ 分析イベント計測基盤（`lib/analytics.ts`、共有ボタンと一括承認に統合済み）

### 実装中・予定
- ⏳ 公開側のリクエスト投稿フォームUI（APIは実装済み）
- ⏳ 公開側の報告フォームUI（APIは実装済み）
- ⏳ 開始/終了報告の自動承認（Cron + ロジック）
- ⏳ 場所変更報告→MoveHint生成→公開側注意＋推定ピン
- ⏳ 他党イベント登録＆表示
- ⏳ 施設データ取り込みスクリプト＋施設レイヤー表示
- ⏳ Umami分析スクリプト統合（`app/layout.tsx`への追加）
- ⏳ テスト（Vitest + Playwright）
- ⏳ 変更履歴の表示UI（データベーススキーマとAPIは実装済み）
...
6. **一括承認機能**: リクエスト画面の一括承認機能は実装済みです（UIとAPI両方）。`app/api/admin/requests/route.ts`の`PATCH`メソッドで実装されています。

7. **他党イベント**: 他党イベントのCRUD機能は未実装です。`app/admin/rival-events/page.tsx` と `app/api/admin/rival-events/route.ts` を実装する必要があります。

8. **共有ボタン**: 共有ボタンは実装済みです（Web Share API + クリップボードコピー）。分析イベント計測も統合済みです。
